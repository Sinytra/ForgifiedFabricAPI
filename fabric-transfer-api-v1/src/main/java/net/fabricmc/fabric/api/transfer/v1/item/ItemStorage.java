...
public final class ItemStorage {
     ...
     *     {@link net.minecraft.world.SimpleContainer} can be used for easy implementation.</li>
    ...
    static {
        ...
        ItemStorage.SIDED.registerFallback((world, pos, state, blockEntity, direction) -> {
            if (blockEntity != null && !TransferApiImpl.COMPUTING_CAPABILITY_LOCK.get()) {
                TransferApiImpl.COMPUTING_CAPABILITY_LOCK.set(true);
                Storage<ItemVariant> storage = blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, direction)
                        .map(ForgeItemStorage::new)
                        .orElse(null);
                TransferApiImpl.COMPUTING_CAPABILITY_LOCK.set(false);
                return storage;
            }
            return null;
        });
