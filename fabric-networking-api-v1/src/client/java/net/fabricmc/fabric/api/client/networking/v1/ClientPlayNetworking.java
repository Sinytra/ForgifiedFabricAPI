...
public final class ClientPlayNetworking {
	 ...
	 * must be performed in the render thread by calling {@link net.minecraft.util.thread.BlockableEventLoop#execute(Runnable)}.
	 *
	...
	public static <T extends FabricPacket> boolean registerGlobalReceiver(PacketType<T> type, PlayPacketHandler<T> handler) {
		return registerGlobalReceiver(type.getId(), new PlayChannelHandlerProxy<T>() {
			...
			@Override
			public void receive(Minecraft client, ClientPacketListener networkHandler, FriendlyByteBuf buf, PacketSender sender) {
				...
				if (client.isSameThread()) {
					...
					client.execute(() -> {
						if (networkHandler.getConnection().isConnected()) handler.receive(packet, client.player, sender);
					});
				}
			}
		});
	...
	public interface PlayPacketHandler<T extends FabricPacket> {
		 ...
		 * @see FabricPacket
		 */
		void receive(T packet, LocalPlayer player, PacketSender responseSender);
	}
}
