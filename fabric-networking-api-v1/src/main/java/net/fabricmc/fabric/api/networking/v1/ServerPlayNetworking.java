 ...
 *
 * <p>This class provides a classic registration method, {@link #registerGlobalReceiver(ResourceLocation, PlayChannelHandler)},
...
public final class ServerPlayNetworking {
	 ...
	 * must be modified in the server thread by calling {@link net.minecraft.util.thread.BlockableEventLoop#execute(Runnable)}.
	 *
	 ...
	 * Use {@link #unregisterReceiver(ServerGamePacketListenerImpl, PacketType)} to unregister the existing handler.
	 *
	 ...
	 * @see ServerPlayNetworking#registerReceiver(ServerGamePacketListenerImpl, PacketType, PlayPacketHandler)
	 */
	public static <T extends FabricPacket> boolean registerGlobalReceiver(PacketType<T> type, PlayPacketHandler<T> handler) {
		return registerGlobalReceiver(type.getId(), new PlayChannelHandlerProxy<T>() {
			...
			@Override
			public void receive(MinecraftServer server, ServerPlayer player, ServerGamePacketListenerImpl networkHandler, FriendlyByteBuf buf, PacketSender sender) {
				...
				if (server.isSameThread()) {
					...
					server.execute(() -> {
						if (networkHandler.isAcceptingMessages()) handler.receive(packet, player, sender);
					});
				}
			}
		});
	 ...
	 * @see ServerPlayNetworking#unregisterReceiver(ServerGamePacketListenerImpl, PacketType)
	 */
	@Nullable
	 ...
	 * Use {@link #unregisterReceiver(ServerGamePacketListenerImpl, ResourceLocation)} to unregister the existing handler.
	 *
	 * <p>For new code, {@link #registerReceiver(ServerGamePacketListenerImpl, PacketType, PlayPacketHandler)}
	 * is preferred, as it is designed in a way that prevents thread safety issues.
	 *
	 ...
	 * @see ServerPlayConnectionEvents#INIT
	 */
	public static <T extends FabricPacket> boolean registerReceiver(ServerGamePacketListenerImpl networkHandler, PacketType<T> type, PlayPacketHandler<T> handler) {
		return registerReceiver(networkHandler, type.getId(), new PlayChannelHandlerProxy<T>() {
			...
			@Override
			public void receive(MinecraftServer server, ServerPlayer player, ServerGamePacketListenerImpl networkHandler2, FriendlyByteBuf buf, PacketSender sender) {
				T packet = type.read(buf);
				...
				if (server.isSameThread()) {
					...
					server.execute(() -> {
						if (networkHandler2.isAcceptingMessages()) handler.receive(packet, player, sender);
					});
				}
			}
		});
	 ...
	 * @return the previous handler, or {@code null} if no handler was bound to the channel,
	 * or it was not registered using {@link #registerReceiver(ServerGamePacketListenerImpl, PacketType, PlayPacketHandler)}
	 */
	@Nullable
	@SuppressWarnings("unchecked")
	public static <T extends FabricPacket> PlayPacketHandler<T> unregisterReceiver(ServerGamePacketListenerImpl networkHandler, PacketType<T> type) {
	 ...
	 */
	public static boolean canSend(ServerPlayer player, PacketType<?> type) {
		Objects.requireNonNull(player, "Server player entity cannot be null");

		return canSend(player.connection, type.getId());
	}
	 ...
	 * @return {@code true} if the connected client has declared the ability to receive a specific type of packet
	 */
	public static boolean canSend(ServerGamePacketListenerImpl handler, PacketType<?> type) {
		Objects.requireNonNull(handler, "Server play network handler cannot be null");
	 ...
	 * @return a new packet
	 */
	public static Packet<ClientGamePacketListener> createS2CPacket(ResourceLocation channelName, FriendlyByteBuf buf) {
	 ...
	 */
	public static <T extends FabricPacket> void send(ServerPlayer player, T packet) {
		Objects.requireNonNull(player, "Server player entity cannot be null");
		...
		packet.write(buf);
		player.connection.send(createS2CPacket(packet.getType().getId(), buf));
	}
	...
	public interface PlayChannelHandler {
		 ...
		 * <pre>{@code
		 * ServerPlayNetworking.registerReceiver(new ResourceLocation("mymod", "boom"), (server, player, handler, buf, responseSender) -> {
	...
	public interface PlayPacketHandler<T extends FabricPacket> {
		 ...
		 * @see FabricPacket
		 */
		void receive(T packet, ServerPlayer player, PacketSender responseSender);
	}
}
