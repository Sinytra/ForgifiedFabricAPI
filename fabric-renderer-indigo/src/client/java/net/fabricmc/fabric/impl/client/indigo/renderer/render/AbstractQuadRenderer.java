...
public abstract class AbstractQuadRenderer {
	...
	AbstractQuadRenderer(BlockRenderInfo blockInfo, Function<RenderType, VertexConsumer> bufferFunc, AoCalculator aoCalc, QuadTransform transform) {
	...
	private void tessellateQuad(MutableQuadViewImpl quad, boolean isVanilla) {
		...
		final RenderType renderLayer = blockInfo.effectiveRenderLayer(mat.blendMode());
	...
	protected void tessellateSmoothEmissive(MutableQuadViewImpl q, RenderType renderLayer, int blockColorIndex) {
		...
		for (int i = 0; i < 4; i++) {
			q.color(i, ColorHelper.multiplyRGB(q.color(i), aoCalc.ao[i]));
			q.lightmap(i, LightTexture.FULL_BRIGHT);
		}
	...
	int flatBrightness(MutableQuadViewImpl quad, BlockState blockState, BlockPos pos) {
		...
		return LevelRenderer.getLightColor(blockInfo.blockView, blockState, mpos);
	}
	...
	private void shadeFlatQuad(MutableQuadViewImpl quad) {
		if (quad.hasVertexNormals()) {
			...
			final float faceShade = blockInfo.blockView.getShade(quad.lightFace(), quad.hasShade());

			for (int i = 0; i < 4; i++) {
				quad.color(i, ColorHelper.multiplyRGB(quad.color(i), vertexShade(quad, i, faceShade)));
			}
		} else {
			final float diffuseShade = blockInfo.blockView.getShade(quad.lightFace(), quad.hasShade());
