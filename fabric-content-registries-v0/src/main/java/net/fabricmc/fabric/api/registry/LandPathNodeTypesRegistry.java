...
public final class LandPathNodeTypesRegistry {
	...
	/**
	 * Registers a {@link BlockPathTypes} for the specified block, overriding the default block behavior.
	 *
	 * @param block              Block to register.
	 * @param nodeType           {@link BlockPathTypes} to associate with the block if it is a direct target
	 *                           in an entity path.
	 *                           (Pass {@code null} to not specify a node type and use the default behavior)
	 * @param nodeTypeIfNeighbor {@link BlockPathTypes} to associate with the block, if it is in a direct neighbor
	...
	/**
	 * Gets the {@link BlockPathTypes} from the provider registered for the specified block state at the specified position.
	 *
	 * <p>If no valid {@link BlockPathTypes} provider is registered for the block, it returns {@code null}.
	 ...
	 * @return the custom {@link BlockPathTypes} from the provider registered for the specified block,
	 ...
	 * the raw provider for the block, if you need the {@link BlockPathTypes} for the block state instead,
	...
	/**
	 * A functional interface that provides the {@link BlockPathTypes}, given the block state.
	 */
	@FunctionalInterface
	public non-sealed interface StaticPathNodeTypeProvider extends PathNodeTypeProvider {
		/**
		 * Gets the {@link BlockPathTypes} for the specified block state.
		 *
		 ...
		 *
		 * <p>For example, for a cactus-like block you should use {@link BlockPathTypes#DAMAGE_CACTUS} if the block
		 * is a direct target in the entity path ({@code neighbor == false}) to specify that an entity should not pass
		 * through or above the block because it will cause damage, and you should use{@link BlockPathTypes#DANGER_CACTUS}
		 ...
		 * @return the custom {@link BlockPathTypes} registered for the specified block state.
		 */
		@Nullable
	...
	/**
	 * A functional interface that provides the {@link BlockPathTypes}, given the block state world and position.
	 */
	@FunctionalInterface
	public non-sealed interface DynamicPathNodeTypeProvider extends PathNodeTypeProvider {
		/**
		 * Gets the {@link BlockPathTypes} for the specified block state at the specified position.
		 *
		 * <p>You can specify what to return if the block state is a direct target of an entity path,
		 * or a neighbor block of the entity path.
		 *
		 * <p>For example, for a cactus-like block you should specify {@link BlockPathTypes#DAMAGE_CACTUS} if the block
		 * is a direct target ({@code neighbor == false}) to specify that an entity should not pass through or above
		 * the block because it will cause damage, and {@link BlockPathTypes#DANGER_CACTUS} if the cactus will be found
		 ...
		 * @return the custom {@link BlockPathTypes} registered for the specified block state at the specified position.
		 */
		@Nullable
