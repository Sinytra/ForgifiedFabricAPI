...
public final class SculkSensorFrequencyRegistry {
	 ...
	 *
	 * <p>Note that the game event must also be in the {@linkplain net.minecraft.tags.GameEventTags#VIBRATIONS} tag to be detected by sculk sensors in the first place.
	...
	public static void register(GameEvent event, int frequency) {
		if (frequency <= 0 || frequency >= 16) {
			throw new IllegalArgumentException("Attempted to register Sculk Sensor frequency for event "+event.getName()+" with frequency "+frequency+". Sculk Sensor frequencies must be between 1 and 15 inclusive.");
		}

		int replaced = VibrationListener.VIBRATION_FREQUENCY_FOR_EVENT.put(event, frequency);

		if (replaced != 0) {
			LOGGER.debug("Replaced old frequency mapping for {} - was {}, now {}", event.getName(), replaced, frequency);
		}
	}
}
