...
public final class TradeOfferInternals {
	...
	private static final Map<VillagerProfession, Int2ObjectMap<List<TradeOffers.Factory>>> VILLAGER_TRADES = new HashMap<>();

	private static final int LEVEL_GENERIC = 1;
	private static final int LEVEL_RARE = 2;
	private static final Int2ObjectMap<List<TradeOffers.Factory>> WANDERING_TRADER_TRADES = new Int2ObjectOpenHashMap<>();
	...
	public static void registerVillagerTrades(VillagerTradesEvent event) {
		Int2ObjectMap<List<TradeOffers.Factory>> trades = VILLAGER_TRADES.get(event.getType());
		if (trades != null) {
	...
	private static void registerOffers(Int2ObjectMap<List<TradeOffers.Factory>> leveledTradeMap, int level, Consumer<List<TradeOffers.Factory>> factory) {
		final List<TradeOffers.Factory> list = new ArrayList<>();
		factory.accept(list);

		final List<TradeOffers.Factory> originalEntries = leveledTradeMap.computeIfAbsent(level, key -> new ArrayList<>());
