...
@Mod("fabric_game_rule_api_v1_testmod")
public class GameRulesTestMod {
		...
		ServerLifecycleEvents.SERVER_STARTED.register(server -> {
			...
			for (CommandNode<CommandSourceStack> child : children) {
				...
				switch (name) {
					...
					default -> throw new AssertionError(String.format("Found unexpected literal name. Found %s but only \"north, south, east, west\" are allowed", name));
				}
			}
