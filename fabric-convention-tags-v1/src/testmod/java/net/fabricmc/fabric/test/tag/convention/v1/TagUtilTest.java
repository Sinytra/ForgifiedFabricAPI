...
public class TagUtilTest {
	...
	public TagUtilTest() {
		ServerLifecycleEvents.SERVER_STARTED.register(server -> {
			if (!TagUtil.isIn(ConventionalEnchantmentTags.INCREASES_BLOCK_DROPS, Enchantments.BLOCK_FORTUNE)) {
				throw new AssertionError("Failed to find fortune in c:fortune!");
			}

			if (TagUtil.isIn(ConventionalBiomeTags.IN_OVERWORLD, server.registryAccess().registryOrThrow(Registries.BIOME).get(Biomes.BADLANDS))) {
			...
			if (!TagUtil.isIn(server.registryAccess(), ConventionalBiomeTags.IN_OVERWORLD, server.registryAccess().registryOrThrow(Registries.BIOME).get(Biomes.BADLANDS))) {
