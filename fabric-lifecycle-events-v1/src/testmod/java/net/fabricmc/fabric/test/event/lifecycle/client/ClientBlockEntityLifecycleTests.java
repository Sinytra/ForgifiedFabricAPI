...
public final class ClientBlockEntityLifecycleTests {
    ...
    private static final List<BlockEntity> clientBlockEntities = new ArrayList<>();
    ...
    public static void onInitializeClient() {
        ...
        ClientBlockEntityEvents.BLOCK_ENTITY_LOAD.register((blockEntity, world) -> {
            clientBlockEntities.add(blockEntity);

            if (PRINT_CLIENT_BLOCKENTITY_MESSAGES) {
                logger.info("[CLIENT]" + " LOADED " + ForgeRegistries.BLOCK_ENTITY_TYPES.getKey(blockEntity.getType()).toString() + " - BlockEntities: " + clientBlockEntities.size());
            }
        });

        ClientBlockEntityEvents.BLOCK_ENTITY_UNLOAD.register((blockEntity, world) -> {
            clientBlockEntities.remove(blockEntity);

            if (PRINT_CLIENT_BLOCKENTITY_MESSAGES) {
                logger.info("[CLIENT]" + " UNLOADED " + ForgeRegistries.BLOCK_ENTITY_TYPES.getKey(blockEntity.getType()).toString() + " - BlockEntities: " + clientBlockEntities.size());
            }
        });
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (clientTicks++ % 200 == 0 && client.level != null) {
                ...
                if (PRINT_CLIENT_BLOCKENTITY_MESSAGES) {
                    logger.info("[CLIENT] Tracked BlockEntities:" + clientBlockEntities.size() + " Ticked at: " + clientTicks + "ticks");
                ...
                if (blockEntities != clientBlockEntities.size()) {
                    if (PRINT_CLIENT_BLOCKENTITY_MESSAGES) {
        ...
        ServerLifecycleEvents.SERVER_STOPPED.register(minecraftServer -> {
            if (!minecraftServer.isDedicatedServer()) { // fixme: Use ClientPlayConnectionEvents#DISCONNECT instead of the server stop callback for testing.
                logger.info("[CLIENT] Disconnected. Tracking: " + clientBlockEntities.size() + " blockentities");

                if (clientBlockEntities.size() != 0) {
